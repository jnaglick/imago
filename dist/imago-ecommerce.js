...
angular.module("imago").run(["$templateCache", function($templateCache) {$templateCache.put("/imago/imago-cart.html","<div class=\"cart\"><div ng-click=\"cart.utils.show = !cart.utils.show\" class=\"icon\"><div ng-bind=\"cart.utils.cart.items.length\" class=\"counter\"></div></div><div ng-show=\"cart.utils.show\" class=\"box\"><div ng-repeat=\"item in cart.utils.cart.items\" class=\"item\"><img ng-if=\"item.serving_url\" ng-src=\"{{item.serving_url}}=s100-c\"/><button ng-click=\"cart.utils.remove(item)\" class=\"remove\"></button><span class=\"qty\">qty: {{item.qty}}</span></div><div ng-show=\"cart.utils.cart.items.length\" class=\"itemnumber\">{{cart.utils.cart.items.length}} items</div><div ng-show=\"cart.utils.cart.items.length === 0\" class=\"noitems\">no items in cart</div><button ng-show=\"cart.utils.cart.items.length\" type=\"submit\" ng-click=\"cart.utils.checkout()\" class=\"checkout\">checkout</button></div></div>");}]);